...
public class StudentController {
    ...
    @Autowired
    private InnovativePracticeRepository innovativePracticeRepository;
    ...
    public List getStudentFeeList(Integer studentId){
        List<Fee> sList =feeRepository.findByStudentStudentId(studentId);  // 查询某个学生消费记录集合
        List list = new ArrayList();
        if(sList == null || sList.size() == 0)
            return list;
        Map m;
        ...
        try {
            ...
            while (rowIterator.hasNext()) {
                ...
                if (!fOp.isPresent()) {
                    ...
                    f.setDay(day);
                    //家人们谁懂啊？！！！
                    Student tempStudent = studentRepository.getById(studentId);
                    f.setStudent(tempStudent);  //不存在 添加
                    //家人们谁懂啊？！！！
                } else {
                    f = fOp.get();  //存在 更新
                }
    ...
    @PostMapping("/getStuDaInformationData")
    @PreAuthorize("hasRole('ROLE_STUDENT')")
    public DataResponse getStuDaInformationData(@Valid @RequestBody DataRequest dataRequest) {
        Integer userId = CommonMethod.getUserId();
        Optional<User> uOp = userRepository.findByUserId(userId);  // 查询获得 user对象
        if(!uOp.isPresent())
            return CommonMethod.getReturnMessageError("用户不存在！");
        User u = uOp.get();
        Optional<Student> sOp= studentRepository.findByPersonPersonId(u.getPerson().getPersonId());  // 查询获得 Student对象
        if(!sOp.isPresent())
            return CommonMethod.getReturnMessageError("学生不存在！");
        Student s= sOp.get();
        Map info = getMapFromStudent(s);  // 查询学生信息Map对象合
        List<DailyActivities> daList = dailyActivitiesRepository.findByStudentStudentId(s.getStudentId());
        //System.out.println(s.getStudentId());
        //Integer l = s.getStudentId();
        Map data = new HashMap();
        data.put("info",info);
        data.put("dailyActivitiesList",getStudentDailyActivitiesList(daList));
        return CommonMethod.getReturnData(data);//将前端所需数据保留Map对象里，返还前端
    }
    public List getStudentDailyActivitiesList(List<DailyActivities> sList){
        List list = new ArrayList();
        if(sList == null || sList.size() == 0)
            return list;
        Map m;
        for(DailyActivities da:sList){
            m = new HashMap();
            m.put("DailyActivitiesId", da.getDailyActivitiesId());
            m.put("studentId", da.getStudent().getStudentId());
            m.put("ipnum", da.getIpnum());
            String ipkinds = da.getIpkinds();
            m.put("ipkinds", ipkinds);
            m.put("ipkindsName", ComDataUtil.getInstance().getDictionaryLabelByValue("ACT", ipkinds));
            m.put("ipname", da.getIpname());
            m.put("iptime",da.getIptime());
            m.put("ipplace", da.getIpplace());
            list.add(m);
        }
        return list;
    }
    public List getStudentInnovativePracticeList(List<InnovativePractice> sList){
        List list = new ArrayList();
        if(sList == null || sList.size() == 0)
            return list;
        Map m;
        for(InnovativePractice ip:sList){
            m = new HashMap();
            ...
            m.put("iporg", ip.getIpplace());
            m.put("iphonor", ip.getIpplace());
            m.put("ipcontent", ip.getIpplace());
            list.add(m);
        }
        return list;
    }
    public List getStudentFeeList(List<Fee> sList){
        List list = new ArrayList();
        if(sList == null || sList.size() == 0)
            return list;
        Map m;
    ...
    @PostMapping("/getStuFeeInformationData")
    @PreAuthorize("hasRole('ROLE_STUDENT')")
    public DataResponse getStuFeeInformationData(@Valid @RequestBody DataRequest dataRequest) {
        Integer userId = CommonMethod.getUserId();
        Optional<User> uOp = userRepository.findByUserId(userId);  // 查询获得 user对象
        if(!uOp.isPresent())
            return CommonMethod.getReturnMessageError("用户不存在！");
        ...
        Student s= sOp.get();
        Map info = getMapFromStudent(s);  // 查询学生信息Map对象合
        List<Fee> fList = feeRepository.findByStudentStudentId(s.getStudentId());
        Map data = new HashMap();
        data.put("info",info);
        data.put("feeList",getStudentFeeList(fList));
        return CommonMethod.getReturnData(data);//将前端所需数据保留Map对象里，返还前端
    }
    @PostMapping("/getStuIpInformationData")
    @PreAuthorize("hasRole('ROLE_STUDENT')")
    public DataResponse getStuIpInformationData(@Valid @RequestBody DataRequest dataRequest) {
        Integer userId = CommonMethod.getUserId();
        Optional<User> uOp = userRepository.findByUserId(userId);  // 查询获得 user对象
        if(!uOp.isPresent())
            return CommonMethod.getReturnMessageError("用户不存在！");
        ...
        Student s= sOp.get();
        Map info = getMapFromStudent(s);  // 查询学生信息Map对象合
        List<InnovativePractice> ipList = innovativePracticeRepository.findByStudentStudentId(s.getStudentId());
        Map data = new HashMap();
        data.put("info",info);
        data.put("innovativePracticeList",getStudentInnovativePracticeList(ipList));
        return CommonMethod.getReturnData(data);//将前端所需数据保留Map对象里，返还前端
    }
