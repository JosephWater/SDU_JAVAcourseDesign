...
@RestController
@RequestMapping("/api/studentAbsent")
public class StuAbsentController {
    //学生提交的请假申请
    @Autowired
    private StuAbsentRepository studentAbsentRepository;  //家庭数据操作自动注入
    @Autowired
    private AbsentRepository absentRepository;  //家庭数据操作自动注入
    @Autowired
    private StudentRepository studentRepository;  //学生数据操作自动注入
    /**
     *  获取 studentAbsent 表的新的Id StringBoot 对SqLite 主键自增支持不好  插入记录是需要设置主键ID，编写方法获取新的 studentAbsent_id
     * @return
     */
    public synchronized Integer getNewStuAbsentId(){
        Integer  id = studentAbsentRepository.getMaxId();  // 查询最大的id
        if(id == null)
            id = 1;
        else
            id = id+1;
     ...
     * @return
     */
    public Map getMapFromStuAbsent(StuAbsent f) {
        Map m = new HashMap();
        Student s;
        Absent e;
        if(f == null)
        ...
        m.put("studentAbsentId", f.getStudentabsentId());
    ...
    /**
     *  getStuAbsentMapList 根据输入参数查询得到学生数据的 Map List集合 参数为空 查出说有学生， 参数不为空，查出人员编号或人员名称 包含输入字符串的学生
     * @param numName 输入参数
     * @return  Map List 集合
     */
    public List getStuAbsentMapList(String numName) {
        List dataList = new ArrayList();
        List<StuAbsent> eList = studentAbsentRepository.findStuAbsentListByNumName(numName);  //数据库查询操作
        if(eList == null || eList.size() == 0)
            return dataList;
        for(int i = 0; i < eList.size();i++) {
            dataList.add(getMapFromStuAbsent(eList.get(i)));
        }
        return dataList;
    }
    /**
     *  getStuAbsent1MapList 根据输入参数查询得到学生数据的 Map List集合 参数为空 查出说有学生， 参数不为空，查出人员编号
     * @param num 输入参数
     * @return  Map List 集合
     */
    public List getStuAbsent1MapList(String num) {
        List dataList = new ArrayList();
        List<StuAbsent> eList = studentAbsentRepository.findByStudentNum(num);  //数据库查询操作
        if(eList == null || eList.size() == 0)
            return dataList;
        for(int i = 0; i < eList.size();i++) {
            dataList.add(getMapFromStuAbsent(eList.get(i)));
    ...
    /**
     *  getStuAbsentList 学生管理 点击查询按钮请求
     *  前台请求参数 numName 学号或名称的 查询串
     * 返回前端 存储学生信息的 MapList 框架会自动将Map转换程用于前后台传输数据的Json对象，Map的嵌套结构和Json的嵌套结构类似
     * @return
     */

    @PostMapping("/getStuAbsentItemOptionList")
    public OptionItemList getStuAbsentItemOptionList(@Valid @RequestBody DataRequest dataRequest) {
        List<StuAbsent> sList = studentAbsentRepository.findStuAbsentListByNumName("");  //数据库查询操作
        OptionItem item;
        List<OptionItem> itemList = new ArrayList();
        for (StuAbsent e : sList) {
    ...
    @PostMapping("/getStuAbsentList")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse getStuAbsentList(@Valid @RequestBody DataRequest dataRequest) {
        String numName= dataRequest.getString("numName");
        List dataList = getStuAbsentMapList(numName);
        return CommonMethod.getReturnData(dataList);  //按照测试框架规范会送Map的list
    }
    /**
     *  getStuAbsent1List 学生管理 点击查询按钮请求
     *  前台请求参数 num 学号 查询串
     * 返回前端 存储学生信息的 MapList 框架会自动将Map转换程用于前后台传输数据的Json对象，Map的嵌套结构和Json的嵌套结构类似
     * @return
     */
    @PostMapping("/getStuAbsent1List")
    public DataResponse getStuAbsent1List(@Valid @RequestBody DataRequest dataRequest) {
        String num= dataRequest.getString("num");
        List dataList = getStuAbsent1MapList(num);
        return CommonMethod.getReturnData(dataList);  //按照测试框架规范会送Map的list
    }
    /**
     * getStuAbsentInfo 前端点击学生列表时前端获取学生详细信息请求服务
     * @param dataRequest 从前端获取 studentId 查询学生信息的主键 studentAbsent_id
     * @return  根据studentAbsentId从数据库中查出数据，存在Map对象里，并返回前端
     */

    @PostMapping("/getStuAbsentInfo")
    public DataResponse getStuAbsentInfo(@Valid @RequestBody DataRequest dataRequest) {
        Integer studentAbsentId = dataRequest.getInteger("studentAbsentId");
        StuAbsent e= null;
        Optional<StuAbsent> op;
        if(studentAbsentId != null) {
            op= studentAbsentRepository.findById(studentAbsentId); //根据学生主键从数据库查询学生的信息
            if(op.isPresent()) {
                e = op.get();
            }
        }
        return CommonMethod.getReturnData(getMapFromStuAbsent(e)); //这里回传包含学生信息的Map对象
    }
    /**
     * StuAbsentEditSave 前端学生请假申请提交服务
     * 前端把所有数据打包成一个Json对象作为参数传回后端，后端直接可以获得对应的Map对象form, 再从form里取出所有属性，复制到
     * 实体对象里，保存到数据库里即可，如果是添加一条记录， id 为空， new StuAbsent 计算新的id， 复制相关属性，保存,通过前端提交给管理员端,使管理员端看到请假信息
     * studentAbsentId不为空。则查询出实体对象，复制相关属性，保存后修改数据库信息，永久修改
     * @return  新建修改学生的主键 student_id 返回前端
     */
    @PostMapping("/studentAbsentEditSave")
    public DataResponse StuAbsentEditSave(@Valid @RequestBody DataRequest dataRequest) {
        Integer studentAbsentId = dataRequest.getInteger("studentAbsentId");
        ...
        Optional<StuAbsent> op;
        if (studentAbsentId != null) {
            op = studentAbsentRepository.findById(studentAbsentId);  //查询对应数据库中主键为id的值的实体对象
            if (op.isPresent()) {
                f = op.get();//获得family
            }
        }
        f = new StuAbsent();   // 创建实体对象
        f.setStudentabsentId(getNewStuAbsentId());//获取新的family主键
        Absent l;
        Student s;
        ...
        studentAbsentRepository.save(f);  // 修改保存请假信息
        return CommonMethod.getReturnData(f.getStudentabsentId());  // 将studentAbsentId返回前端

    }
    @PostMapping("/absentreq")
    public DataResponse absentreq(@Valid @RequestBody DataRequest dataRequest) {
        Integer stuabsentId = dataRequest.getInteger("studentAbsentId");
